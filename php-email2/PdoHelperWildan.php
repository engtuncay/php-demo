...
//https://github.com/wildantea/php-pdo-mysql-helper-class/blob/master/Database.php
...
class PdoHelperWildan
{
  ...
  public function roleUserMenu()
  {
    ...
    foreach ($this->query("select sys_menu.url from sys_menu inner join sys_menu_role on sys_menu.id=sys_menu_role.id_menu
          where hide='N' and sys_menu_role.group_level=? and sys_menu_role.read_act=?", array('sys_menu_role.group_level' => $_SESSION['group_level'], 'sys_menu_role.read_act' => 'Y')) as $role) {
      $role_user[] = $role->url;
    }
    return $role_user;
  }
